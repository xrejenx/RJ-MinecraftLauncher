...
static bool isVersionNewer(const QString &current, const QString &latest) {
    ...
    int len = std::max(curParts.size(), latParts.size());
    
    for (int i = 0; i < len; ++i) {
        QString c = i < curParts.size() ? curParts[i] : "0";
        QString l = i < latParts.size() ? latParts[i] : "0";
        
        bool cOk, lOk;
        int cNum = c.toInt(&cOk);
        int lNum = l.toInt(&lOk);
        ...
        if (idx == 0) {
            if (rbUpdateYes->isChecked()) {
                ...
                if (!ui.allReleases.isEmpty()) {
                    QString latest = ui.allReleases.first().tagName;
                    if (isVersionNewer(GetAppVersion(), latest)) {
