...
private slots:
    void startDownload() {
        ...
        if (os == "windows") {
            libUrl = "https://raw.githubusercontent.com/xrejenx/RJLauncher/RJL/assets/7zip/Windows7zip/7za.exe";
            destName = "7za.exe";
        } else {
            libUrl = "https://raw.githubusercontent.com/xrejenx/RJLauncher/RJL/assets/7zip/Linux7zip/7za";
            destName = "7za";
        }
...
public:
    UpdateCoreUI(QWidget *parent = nullptr) : QWidget(parent) {
            ...
            for (const QString &pkg : DetectDownloadedPackages()) {
                ...
                QObject::connect(delBtn, &QPushButton::clicked, [pkg, localZips, btnInstallLocal]() {
                    if (QMessageBox::question(nullptr, "Delete", "Remove " + pkg + "?") == QMessageBox::Yes) {
                        QString dataRoot = MinecraftLauncher::getRJLDataPath();
                        QString pkgPath = dataRoot + "LauncherUpdater/LauncherSource/" + pkg;
                        
                        if (QFile::exists(pkgPath)) QFile::remove(pkgPath);
        ...
        QListWidget *rollList = new QListWidget(this);
        rollLayout->addWidget(rollList);

        QPushButton *btnRollbackMain = new QPushButton("Restore Selected Backup", this);
        btnRollbackMain->setEnabled(false);
        rollLayout->addWidget(btnRollbackMain);
        ...
        auto applyRollFilter = [rollList, searchRoll]() {
            ...
            for (int i = 0; i < rollList->count(); ++i) {
                ...
                QWidget *w = rollList->itemWidget(item);
                if (w) {
                    QString name = item->data(Qt::UserRole).toString();
                    item->setHidden(!name.contains(text, Qt::CaseInsensitive));
                }
        ...
        auto refreshRollbackTab = [rollList, btnRollbackMain, applyRollFilter, this]() {
            rollList->clear();
            btnRollbackMain->setText("Restore Selected Backup");
            btnRollbackMain->setEnabled(false);
            
            QStringList backups = DetectRollbackPackages();
            for (const QString &oldFile : backups) {
                QListWidgetItem *item = new QListWidgetItem(rollList);
                item->setSizeHint(QSize(0, 38));
                item->setData(Qt::UserRole, oldFile);

                QWidget *row = new QWidget();
                QHBoxLayout *rowLayout = new QHBoxLayout(row);
                rowLayout->setContentsMargins(10, 2, 10, 2);

                QLabel *nameLbl = new QLabel(oldFile, row);
                rowLayout->addWidget(nameLbl, 1);

                QPushButton *rollBtn = new QPushButton("Rollback", row);
                rollBtn->setFixedWidth(75);
                rollBtn->setVisible(false);
                rowLayout->addWidget(rollBtn);
                ...
                rollList->setItemWidget(item, row);

                // Action: Rollback
                QObject::connect(rollBtn, &QPushButton::clicked, [oldFile]() {
                    TriggerRollback(oldFile);
                });

                // Action: Delete
                QObject::connect(delBtn, &QPushButton::clicked, [oldFile, rollList]() {
                    auto reply = QMessageBox::question(nullptr, "Delete Backup", "Permanently remove " + oldFile + "?", 
                                                       QMessageBox::Yes | QMessageBox::No);
                    if (reply == QMessageBox::Yes) {
                        QString dataRoot = MinecraftLauncher::getRJLDataPath();
                        QString fullPath = dataRoot + "LauncherUpdater/LauncherSource/old/" + oldFile;
                        ...
                        for (int i = 0; i < rollList->count(); ++i) {
                            QListWidgetItem *item = rollList->item(i);
                            if (item && item->data(Qt::UserRole).toString() == oldFile) {
                                delete rollList->takeItem(i);
                                break;
                            }
                        }
                    }
        ...
        // Selection handling: Show/Hide buttons and enable main button
        connect(rollList, &QListWidget::currentItemChanged, [rollList, btnRollbackMain](QListWidgetItem *curr, QListWidgetItem *prev) {
            if (prev) if (QWidget *w = rollList->itemWidget(prev)) {
                for (auto *btn : w->findChildren<QPushButton*>()) btn->hide();
            }
            if (curr) if (QWidget *w = rollList->itemWidget(curr)) {
                for (auto *btn : w->findChildren<QPushButton*>()) btn->show();
            }
            btnRollbackMain->setEnabled(curr != nullptr);
        });

        connect(btnRollbackMain, &QPushButton::clicked, [rollList]() {
            QListWidgetItem *item = rollList->currentItem();
            if (item) {
                TriggerRollback(item->data(Qt::UserRole).toString());
            }
        });
        ...
        connect(updateTabs, &QTabWidget::currentChanged, [updateTabs, refreshTab3, refreshRollbackTab](int index) {
