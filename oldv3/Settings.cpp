...
public:
    explicit SettingsDialog(QWidget *parent = nullptr) : QDialog(parent) {
        ...
        // RJtweak Tab (Console & Log Toggles)
        QWidget *tweakTab = new QWidget();
        QVBoxLayout *tweakLayout = new QVBoxLayout(tweakTab);
        tweakLayout->addWidget(new QLabel("<b>RJtweak - Advanced Logging</b>"));

        QCheckBox *showLogs = new QCheckBox("Show logs (Popup CMD Menu)", tweakTab);
        showLogs->setChecked(settings.value("launcher/showConsole", false).toBool());
        connect(showLogs, &QCheckBox::toggled, this, [](bool checked) {
            QString dataRoot = MinecraftLauncher::getRJLDataPath();
            QSettings s(dataRoot + "launcher.ini", QSettings::IniFormat);
            s.setValue("launcher/showConsole", checked);
            EnsureConsoleVisibility();
        });
        tweakLayout->addWidget(showLogs);
        tweakLayout->addStretch();

        tabWidget->addTab(generalTab, "General");
        tabWidget->addTab(connectionTab, "Connection");
        tabWidget->addTab(tweakTab, "RJtweak");
