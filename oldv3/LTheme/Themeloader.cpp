...
void ShowCreateThemeDialog(QWidget *parent, const std::function<void()> &onCreated) {
    ...
    auto updatePreview = [preview, colorEdit]() {
        QString col = colorEdit->text().trimmed();
        if (!col.startsWith("#")) col = "#" + col;
        if (QColor::isValidColorName(col)) {
            preview->setStyleSheet(QString("background-color: %1; border: 1px solid gray;").arg(col));
        }
    };
    updatePreview();
    QObject::connect(colorEdit, &QLineEdit::textChanged, updatePreview);

    QHBoxLayout *colorLayout = new QHBoxLayout();
    colorLayout->addWidget(preview);
    ...
    layout->addRow("Background Color:", colorLayout);
...
void ThemeLoader::initialize() {
    ...
    QString themesPath = dataRoot + "LTheme/themes";
...
void ThemeLoader::createDefaultThemes() {
    ...
    for (const QString &name : defaults) {
        ...
        // Detect if folder exists - if not, create it
        if (!QDir(path).exists()) {
...
void ThemeLoader::createCustomTheme(const CustomThemeInfo &info) {
    ...
    theme["text"] = info.autoDetect ? "#000000" : "#D4D4D4"; // Black for light, light gray for dark
    theme["accent"] = info.autoDetect ? "#0078D7" : "#2D2D2D"; // Blue for light, dark gray for dark
    theme["border"] = info.autoDetect ? "#CCCCCC" : "#454545"; // Light gray for light, dark gray for dark
    theme["background_image"] = info.imagePath;
    theme["bta_css_color"] = theme["accent"]; // Button color is accent color
    theme["bta_css_curve"] = "0";
